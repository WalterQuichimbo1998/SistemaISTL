...
public class MatriculaFacade extends AbstractFacade<Matricula> {
    ...
    public Matricula obtenerMatricula2(String cedula) {
        ...
        try {
            Query q = em.createNativeQuery("SELECT datos_personales.id_datos_personales,matricula.id_matricula FROM datos_personales \n"
                    ...
                    + "WHERE datos_personales.num_identificacion = '" + cedula + "';", Matricula.class);
            ma = (Matricula) q.getSingleResult();
        } catch (Exception e) {
        }
        return ma;
    ...
    public Matricula totalMatriculas(Integer id) {
        Matricula ma = null;
        try {
            Query q = em.createNativeQuery("SELECT * FROM matricula \n"
                    + " WHERE id_periodo_academico='" + id + "'ORDER BY numero_formulario DESC LIMIT 1;", Matricula.class);
            ma = (Matricula) q.getSingleResult();
        } catch (Exception e) {
        }
        return ma;
    ...
    public Matricula totalMatriculas2(Integer id) {
        Matricula ma = null;
        try {
            Query q = em.createNativeQuery("SELECT * FROM matricula \n"
                    + " WHERE id_periodo_academico='" + id + "'ORDER BY numero_formulario DESC LIMIT 1;", Matricula.class);
    ...
    public List<Matricula> totalMatriculasBuscar(Integer id) {
            Query q = em.createNativeQuery("SELECT * FROM matricula \n"
                    + " WHERE id_periodo_academico='" + id + "';", Matricula.class);
           List<Matricula> lista = q.getResultList();
        return lista; 
    }
